...
class cmdmaker:
    ...
    def __init__(self,washa,washaName):
        ...
        if os.path.isdir('./voiceConfig'):
            if os.path.exists("voiceConfig/" + self.washaName + ".csv"):
                csvlist = getcsv("voiceConfig/" + self.washaName + ".csv")
                csvlist.pop(0)
                for cho in csvlist:
                    for chousei in self.chouseiConfig:
                        if str(chousei[0]) == str(cho[0]):
                            self.chouseiConfig[int(chousei[0])] = cho
                            break
    ...
    def getTaskList(self,tlist):
        ...
        self.Jimakulist.append(tlist)
        
    def getInit(self):
        return self.washaInit
                ...
                if not self.stop_Talk:
                    for hoge in manager.get_store_list:
                        if hoge[0] == self.short_ID:
                            ...
                            print(manager.get_store_list)
                        while True:
                            if self.stop_Talk:
                                print("open")
                                break
                            print("block:" + self.washaName)
                            time.sleep(1)
    ...
    def getspeeknow(self):
        return self.speeknow
    
    def getJimakuList(self):
        return self.Jimakulist
    
    def popJimakulist(self):
        self.Jimakulist.pop(0)
        
    def getJlistlen(self):
        return len(self.Jimakulist)
    ...
    def DrawMyChar(self):
        ...
        #初期化
            ...
            while True:
                ...
                if face is not self.faceconfig and not speekNow:
                    vanishtimer = 1
                    if self.faceconfig < faceMax:
                        face = self.faceconfig
                    else:
                ...
                if speekNow:
                    ...
                    if len(anime_talk[face]) > 0:
                        if animationNumber >= len(anime_talk[face]):
                            animationNumber = 0
                        maskA = anime_talk[face][animationNumber][:,:,3].copy()
                    ...
                    if len(anime_wait[face]) > 0:
                        if animationNumber >= len(anime_wait[face]):
                            animationNumber = 0                 
                        maskA = anime_wait[face][animationNumber][:,:,3].copy()
                        ret,maskA = cv2.threshold(maskA, 1, 1, cv2.THRESH_BINARY)
                        maskA = cv2.cvtColor(maskA, cv2.COLOR_GRAY2BGR)
                        needchange = True
...
class MyFrame(wx.Frame):
    def __init__(self, parent, title):
        ...
        panel1_1.SetBackgroundColour(wx.WHITE)
        panel1_2 = wx.Panel(panel1, wx.ID_ANY, size=(720, 50))
        panel1_2.SetBackgroundColour(wx.WHITE)
        panel1_3 = wx.Panel(panel1, wx.ID_ANY, size=(720, 50))
        panel1_3.SetBackgroundColour(wx.WHITE)        
        panel1_4 = wx.Panel(panel1, wx.ID_ANY, size=(720, 50))
        panel1_4.SetBackgroundColour(wx.WHITE)
        panel1_5 = wx.Panel(panel1, wx.ID_ANY, size=(720, 50))
        panel1_5.SetBackgroundColour(wx.WHITE)   
        panel1_6 = wx.Panel(panel1, wx.ID_ANY, size=(720, 50))
        panel1_6.SetBackgroundColour(wx.WHITE)
        panel1_7 = wx.Panel(panel1, wx.ID_ANY, size=(720, 40))
        panel1_7.SetBackgroundColour(wx.WHITE)
        panel1_8 = wx.Panel(panel1, wx.ID_ANY, size=(720, 160))
        panel1_8.SetBackgroundColour(wx.WHITE)   
        panel1_9 = wx.Panel(panel1, wx.ID_ANY, size=(720, 40))
        panel1_9.SetBackgroundColour(wx.WHITE)   
        panel1_10 = wx.Panel(panel1, wx.ID_ANY, size=(720, 40))
        panel1_10.SetBackgroundColour(wx.WHITE) 
        panel1_11 = wx.Panel(panel1, wx.ID_ANY, size=(720, 40))
        panel1_11.SetBackgroundColour(wx.WHITE)   
        ...
        layout1_7.Add(self.button1_7_1,flag = wx.GROW | wx.LEFT, border=120)
        layout1_7.Add(self.button1_7_2,flag = wx.GROW | wx.RIGHT, border=120)
